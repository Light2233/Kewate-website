...
>   
    ...
    <div class="image-bg2" class:image-bg1={page != "multi-page"} class:image-bg2={page == "multi-page"}>
    </div>
    {#if page == "landing"}
        <video id="myVideo" preload="none" autoplay loop muted playsinline>
            <source src="{images[0]}" type='video/mp4;codecs="avc1.42E01E, mp4a.40.2,h264'>
        </video>
    {:else}
        <img src="{images[0]}" alt="" class:bg1={page == "multi-page" } class:bg2={page == "store" }>
        <img src="{images[2]}" alt="" class:laptop={page == "multi-page"} class:laptop2={page == "store"}>
    {/if}
    {#key isInView}
        <div class="tagline_info_bottom">
            ...
            {#if innerWidth < 600}
                ...
                <!-- <button type="button" class="main_sm_16 main_btn_white" on:click={()=>{showModal=true}}>Обсудить задачу</button> -->
            {/if}
        </div>
    {/key}
